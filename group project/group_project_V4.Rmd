...
## Betweenness Centrality plot with label
```{r}
set.seed(666)
ggraph(network_graph, layout="fr") +geom_edge_link(arrow = arrow(length = unit(1,'mm')),end_cap = circle(1.2,'mm'))+geom_node_point(aes(size=bc, color=nodes$wg), show.legend=T)
```
## Eigenvector Centrality plot with label
```{r}
set.seed(666)
ggraph(network_graph, layout="fr") +geom_edge_link(arrow = arrow(length = unit(1,'mm')),end_cap = circle(1.2,'mm'))+geom_node_point(aes(size=ec, color=nodes$wg), show.legend=T)
```
...
vertices %>%  ggplot(aes(x = dc, y = app_proc_time, color = gender)) + geom_point()

vertices %>%  ggplot(aes(x = dc, y = app_proc_time, color = race)) + geom_point()

vertices %>%  ggplot(aes(x = tenure_days, y = app_proc_time, color = race)) + geom_point()
